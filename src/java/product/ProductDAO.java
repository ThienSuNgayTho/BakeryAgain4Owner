...
public class ProductDAO implements Serializable {
    ...
    public void getAvailableProductList(int index)
        ...
        try {
            ...
            if (con != null) {
                //2.Create sQL String
                String sql = "Select p.ProductId, p.ProductName, p.CategoryId, c.CategoryName, p.OringinalSalePrice, "
...
//                    Category c = getProductCategory(categoryId);
    ...
    public int getMaxPagesSearchNameBy8(String searchValue) throws SQLException, NamingException {
        ...
        try {
            ...
            if (con != null) {
                String sql = "select count(*) from Product "
                        + "Where Status = ? and ProductName Like ? ";
...
//                    Category c = getProductCategory(categoryId);
    ...
    public int getMaxProductId() throws SQLException, NamingException {
        ...
        try {
            ...
            String sql = "Select MAX(ProductId) "
                    + "From Product ";
        ...
        try {
            ...
            if (con != null) {
                ...
                String sql = "Insert Into "
                        + "Product (ProductName, CategoryId, ThumbnailLink, OringinalSalePrice, Details, SalePrice, Status, Stock, SaleStatus) "
                        + "Values(?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
//                        + "Where Id = ?";
                ...
                String sql = "UPDATE Product "
                        + "SET ProductName = ?, CategoryId = ?, OringinalSalePrice = ?, Details = ?, SalePrice = ?, Status = ?, Stock = ?, SaleStatus = ?, ThumbnailLink = ? "
                        + "Where ProductId = ?";
                //3. Create Statement and assign Parameter (if any)
                stm = con.prepareStatement(sql);
                stm.setString(1, productName);
    ...
    public boolean isAvailable(int productID) {
        ...
        try {
            conn = new DbConnect().makeConnection();
            String sql = "SELECT Status, Stock FROM [Product] WHERE ProductId = ? ";
            ps = conn.prepareStatement(sql);
            ps.setInt(1, productID);
            rs = ps.executeQuery();
            while (rs.next()) {
